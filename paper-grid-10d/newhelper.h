...
struct specialind{
  individual *point; //the individual's basic stats.
    int sorder[DIMENSIONS];
    //int xorder, yorder, zorder, korder; //Position of the individual in its respective sorting lists
};
...
static inline bool specialkcomparator(specialind * A, specialind * B){
    return A->point->f[3] < B->point->f[3];
}
...
//New algorithm, one dimension probability accumulate function 'chi' ******************************
static inline double chi(double cellcorner, double mu, double s){
    double temp;
    temp = (cellcorner-mu)/s;
    return s * ( gausspdf(temp) + temp*gausscdf(temp) );
}



//Comparator function for sorting the inviduals in ascending order of x coordinate.
static inline bool xcomparator(individual * A, individual * B){
  return A->f[0] < B->f[0];
}

//Comparator function for sorting the inviduals in ascending order of y coordinate.
static inline bool ycomparator(individual * A, individual * B){
  return A->f[1] < B->f[1];
}

//Comparator function for sorting the inviduals in ascending order of z coordinate.
static inline bool zcomparator(individual * A, individual * B){
  return A->f[2] < B->f[2];
}/* 
