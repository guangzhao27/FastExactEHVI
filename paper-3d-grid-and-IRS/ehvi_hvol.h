//Header to be included in order to use the hypervolume calculations implemented
//in ehvi_hvol.cc
#include "newhelper.h"
#include <deque>
using namespace std;
...
//the hypervolume should be calculated. Points outside this box are not ignored,
...
//correct answer.
double hvol3d(deque<individual*> P, double cl[DIMENSIONS], double fmax[DIMENSIONS]);

//Calculates the 2D slice between r and fmax, flattened in dimension 'dimension'.
double calculateslice(deque<individual*> P, double r[DIMENSIONS], double fmax[DIMENSIONS], int dimension);
