...
// The previous version, the normal
//Probability density function for the truncated normal distribution.
static inline double gausspdf(double x, double mu, double s){
  double a=(x-mu)/s;
  return SQRT_TWOPI_NEG * exp(-(a*a)/2);
}

//Cumulative distribution function for the normal distribution
static inline double gausscdf(double x, double mu, double s){
  double a=(x-mu)/s;
  return 0.5*(1+erf(a/SQRT_TWO));
}

//Partial expected improvement function 'psi'.
static inline double exipsi(double fmax, double cellcorner, double mu, double s){
 return s * gausspdf(cellcorner, mu, s) + (mu-fmax) * (1-gausscdf(cellcorner, mu, s));
}
